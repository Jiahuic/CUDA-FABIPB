...
- because `panelIA0()` and the quadrature rules are still CPU-side, a true
  on-the-fly non-stored direct GPU path is not attractive yet
- without GPU quadrature, an on-the-fly path would create repeated CPU
  coefficient generation plus repeated CPU-GPU transfers
- therefore the next direct-sum comparison path should be a
  destination-block / row-block direct GPU design rather than a fully on-the-fly
  direct GPU design
...
## Workstream A next implementation path

The direct-sum comparison should proceed in two steps.

### Step A1: destination-block direct GPU reference

Build a direct GPU reference path that avoids storing the full dense operator.

Design:

- choose a destination panel block size
- for each destination block:
  - compute panel-panel coefficients for that block
  - upload just that block to GPU
  - apply the block matvec on GPU
  - accumulate into the destination output
  - discard the temporary block coefficients

Benefits:

- avoids the full `O(n^2)` device-memory barrier
- allows larger direct-sum reference runs than the current dense cached path

Limitation:

- the coefficients are still generated by CPU `panelIA0()`
- so this is primarily a memory-feasibility improvement, not a final
  high-performance direct PB solver

### Step A2: evaluate whether GPU quadrature is worth pursuing

After the destination-block path exists, measure:

- coefficient-generation time on CPU
- upload time
- GPU apply time

If CPU coefficient generation dominates, then the direct-sum line of work only
becomes seriously competitive after porting quadrature to GPU.

If the blocked direct path is already informative enough as a reference, then it
may be sufficient for the paper without pushing full GPU quadrature.

## Immediate next steps

1. Use the in-tree direct GPU baseline on small cases that fit memory.
2. Design and implement a destination-block direct GPU reference path.
3. If needed, retrieve the older direct-sum PB solver and compare against it.
4. Extend the benchmark matrix outputs into a depth-selection study.
5. Add a report-only `nLev` recommendation mode in this repo after the direct
