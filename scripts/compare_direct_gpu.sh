...
./scripts/with_benchmark_env.sh "$BUILD_DIR/fabipb" -B=1 -g=1 -r=1 -m=0 "$panel" "$@" >"$direct_log" 2>&1
./scripts/with_benchmark_env.sh "$BUILD_DIR/fabipb" -B=1 -g=1 -m=0 "$panel" "$@" >"$fmm_gpu_log" 2>&1
